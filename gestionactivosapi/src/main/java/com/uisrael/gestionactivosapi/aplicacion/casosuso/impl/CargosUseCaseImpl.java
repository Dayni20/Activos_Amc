...
public class CargosUseCaseImpl implements ICargosUseCase {
	...
	public Cargos obtenerPorId(int id) {
		return repositorio.buscarPorId(id).orElseThrow(() -> new RuntimeException("Cargo no encontrado"));
	}
	...
	public Cargos actualizarEstado(int id, boolean estado) {
		Cargos actual = repositorio.buscarPorId(id).orElseThrow(() -> new RuntimeException("Cargo no encontrado"));
